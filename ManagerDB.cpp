...
void ManagerDB::resize() {
    ...
    Manager* temp = new Manager[maxlength * 2];
...
int ManagerDB::findElement(std::string id) const {
    if (getSize() == 0) {   // empty Database
        ...
        for (int i = 0; i < getSize(); data) {
            if (data[i].getManagerID() == id) { // if ID (Customer, Sales, Manager) or name (Order, Product, Service) are equal
                return i;
            }
        }
    }
...
void ManagerDB::add(Manager newElement) {
    ...
    if (size == 0) {
        dbfile << "Manager" << std::endl;    // writes the type to the file
    }
...
bool ManagerDB::remove(Manager element) {       // not complete yet
...
void ManagerDB::loadFromFile() {
    ...
    while (std::getline(dbfile, line)) { // read each line of the file, until end of the file
        ...
        m = Manager(username, password, acc, id);
